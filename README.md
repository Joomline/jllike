# JL Like

**Пакет плагинов социальных кнопок для Joomla 4, 5 и 6**

---

## Описание

JL Like — это набор плагинов для вывода современных социальных кнопок "Поделиться" в материалах Joomla и популярных компонентах:

- Content (статьи Joomla)
- K2
- ZOO
- EasyBlog
- VirtueMart
- JoomShopping
- ADSmanager

Плагин поддерживает популярные соцсети: Facebook, ВКонтакте, X (Twitter), OK.ru, Мой Мир, LinkedIn, Pinterest, LiveJournal, Blogger, Weibo, Viber, WhatsApp, Telegram, Threads, Reddit и др.

---

## Совместимость

- **Joomla 6.x** ✨ NEW
- **Joomla 5.x**
- **Joomla 4.x**
- PHP 8.2+ (рекомендуется)
- PHP 8.1+ (минимум)
- Актуальная версия пакета: **5.3.0** (от 23.01.2026)

---

## Установка

1. Установите пакет через стандартный менеджер расширений Joomla.
2. Включите нужные плагины в разделе "Расширения → Плагины":
    - JL Like (content)
    - JL Like K2
    - JL Like for JoomShopping
    - JL Like for ADSmanager
    - JL Like for VirtueMart и др.
3. Настройте параметры отображения и автодобавления кнопок в каждом плагине.
4. Для ручного вывода используйте тег `{jllike}` в тексте материала.

---

## Обновления и миграция

### Версия 5.3.0 (23.01.2026)

**Новые возможности:**
- ✨ Добавлено 5 готовых стилей кнопок (Default, Minimal, Gradient, Outlined, Floating)
- ✨ Система Drag-and-Drop для управления порядком кнопок социальных сетей
- ✨ Добавлена поддержка Threads и Reddit
- ✨ Система сворачивания кнопок с настраиваемой кнопкой "Ещё"
- ✨ Интерактивный виджет предпросмотра в настройках плагина

**Улучшения:**
- ⚡ Обновлен логотип и цвет Twitter на X
- ⚡ Улучшено позиционирование фиксированных кнопок (слева по вертикальному центру)
- ⚡ Динамическая ширина и центрирование текста кнопки "Ещё"
- ⚡ Расширены CSS-стили (добавлено 285+ строк)

**Для обновления:**
- Для обновления с предыдущих версий просто установите новый пакет поверх старого через менеджер расширений Joomla.

### Версия 5.2.0 (12.12.2025)

**Улучшения совместимости:**
- ✅ Полная совместимость с Joomla 6
- ✅ Совместимость с PHP 8.2+
- ✅ Удалены устаревшие API (jimport, JLoader, ContentHelperRoute)
- ✅ Обновлены все методы для современного Joomla API
- ✅ Исправлены критические баги и логические ошибки
- ✅ Замена `var` на `public`/`protected` для PHP 8.2+
- ✅ Использование Input API вместо прямого доступа к $_COOKIE
- ✅ Обновлен Query Builder для работы с базой данных

---

## Официальные страницы

- [Страница расширения (RU)](https://joomline.ru/rasshirenija/plugin/jllike.html)
- [Страница расширения (EN)](http://joomline.org/extensions/scripts-other-developments/jllike.html)
- [Страница на JED](https://extensions.joomla.org/extension/joomline-like)

---

## Лицензия

GPL


![GitHub all releases](https://img.shields.io/github/downloads/joomline/jllike/total?style=for-the-badge&color=blue)  ![GitHub release (latest by SemVer)](https://img.shields.io/github/downloads/Joomline/jllike/latest/total?style=for-the-badge&color=blue)